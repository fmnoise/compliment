...
(defn- ns-candidates
  ...
  (for [[alias ns] (ana/ns-aliases env context-ns)]
    (merge
     (candidate-data alias (when-not (= alias ns) ns) :namespace)
     (when (seq extra-metadata)
       (candidate-extra extra-metadata (meta (find-ns ns)))))))
...
(defn- referred-var-candidates
  ...
  [env context-ns extra-metadata]
  (for [[refer qualified-name] (ana/referred-vars env context-ns)
        :let [ns (namespace qualified-name)
              meta (ana/find-symbol-meta env qualified-name)
              type (var->type meta)]]
    (merge
...
(defn- referred-macro-candidates
  ...
  [env context-ns extra-metadata]
  (for [[refer qualified-name] (ana/referred-macros env context-ns)
        :let [ns (namespace qualified-name)
              meta #?(:clj (ana/var-meta (find-var qualified-name))
                      :cljs (let [referred-ns (symbol (namespace qualified-name))]
                              (-> env
                                  (ana/ns-interns-from-env (ana/add-ns-macros referred-ns))
                                  (get refer)
                                  ana/var-meta)))]]
    (merge (candidate-data refer ns :macro)
           (when (seq extra-metadata)
             (candidate-extra extra-metadata meta)))))
